...
// icmpv6CantFragment is an error where a packet requires
// fragmentation as per RFC 4443 page 9, Packet Too Big Message.
type icmpReasonCantFragment struct{}

func (*icmpReasonCantFragment) isICMPReason() {}

func (*icmpReasonCantFragment) isForwarding() bool {
	// If we hit a Packet Too BBig error, then we know we are operating
	// as a router. As per RFC 4443 section 3.2:
	//
